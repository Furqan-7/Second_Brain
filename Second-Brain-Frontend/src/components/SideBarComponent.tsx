  ...
  return (
    <div className="flex flex-col w-20 md:w-56 lg:w-72 min-h-screen border border-gray-300 shadow-xl/30 sticky top-0 self-start">
      <div className="flex items-center mt-5 ml-5 mr-5 gap-2">
        ...
        <p className="font-semibold text-[19px] hidden md:flex">Second Brain</p>
      </div>
...
function SideBarIcons({ Icon, Text, color }: SideBarProps) {
  return (
    <div className=" hover:bg-blue-100 hover:cursor-pointer flex items-center rounded-[10px] p-2 ml-4 mr-8 mt-4 gap-3">
      <Icon color={color} className="md:size-5" />
      <p className="text-[#475569] text-[15px] hidden md:flex tracking-wide">
        {Text}
      </p>
    </div>
  );
